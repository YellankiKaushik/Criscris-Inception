...
export type ScoreBand =
  "Strong Response" | "Effective Response" | "Needs Improvement" | "High-Risk Response";
...
export function calculateScore(state: SimulationState): ScoreBreakdown {
  ...
  if (report) {
    if (report.timestampSeconds <= 45) {
      ...
      improvements.push(
        "You reported the emergency late; reporting should happen within the first minute.",
      );
    }
  } else {
  ...
  if (fireControl) {
    if (fireControl.stage === "low") {
      ...
      riskAssessment -= 8;
      improvements.push(
        "You attempted direct fire control after conditions had already escalated.",
      );
    } else {
      riskAssessment -= 15;
      improvements.push(
        "You attempted direct fire control after the scenario reached critical conditions.",
      );
    }
  } else {
  ...
  if (evacuate) {
    ...
    improvements.push(
      "You never evacuated; the scenario ended with you still inside the building.",
    );
  }
  ...
  situationalAwareness = clamp(situationalAwareness, 20);
  emergencyReporting = clamp(emergencyReporting, 20);
  riskAssessment = clamp(riskAssessment, 20);
  responseTime = clamp(responseTime, 20);
  evacuationDecision = clamp(evacuationDecision, 20);
